      ...
      link: function(scope, element, attrs) {
        ...
        scope.renderPage = function(num) {
          ...
          pdfDoc.getPage(num).then(function(page) {
            ...
            console.log(reason);
            if (typeof scope.onPageError === 'function') {
              scope.onPageError();
            }
          });
        };
        ...
        function renderPDF() {
          ...
          } else if (pdfArr && pdfArr.length) {
            pdfLoaderTask = PDFJS.getDocument(pdfArr);
            pdfLoaderTask.onProgress = scope.onProgress;
            //pdfLoaderTask.onPassword = scope.onPassword;
            pdfLoaderTask.then(
              function(_pdfDoc) {
        ...
        scope.$watch('pdfArr', function(newVal) {
          if (newVal !== '') {
            if (debug) {
              console.log('pdfArr value change detected: ', scope.pdfArr);
            }
            pdfArr = newVal;
            scope.pageNum = scope.pageToDisplay = pageToDisplay;
            if (pdfLoaderTask) {
