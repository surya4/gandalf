noonEduController
  .controller('VoucherAddonCtrl', ['$rootScope','$state','ngProgressFactory','APICall','Config','$stateParams',
    function ($rootScope,$state,ngProgressFactory,APICall,Config,$stateParams) {
      ...
      vm.getVoucherAddonIncluded = function () {
        vm.progressbar.start();
        var url = Config.PACKAGE_SRV + 'voucherAddons/' + vm.voucherId;
        APICall.getAPIData(url, {}, Config.API_METHOD_TYPE.GET).then(function (response) {
            vm.voucherAddons=response.data;
            vm.progressbar.complete();
            vm.getAddonList();
          },
          function (response) {
      ...
      vm.checkIncluded = function (key, addon_id) {
       //check if exist and change addon.included
        for(var i in  vm.voucherAddons){
          if(vm.voucherAddons[i].id==addon_id){
      ...
      vm.toggleIncluded = function (key, addon_id, included) {
        vm.progressbar.start();
        var url = Config.PACKAGE_SRV + 'toggleVoucherAddons';
        var data={voucher_id:vm.voucherId,addon_id:addon_id,included:included};
        APICall.getAPIData(url, data, Config.API_METHOD_TYPE.POST).then(function (response) {
            if(included){
              vm.voucherAddons.push(data);
              vm.addons[key].included=true;
            } else {
              for(var i in  vm.voucherAddons){
                if(vm.voucherAddons[i].id==addon_id){
                  delete vm.voucherAddons[i];
                  vm.addons[key].included=false;
                  break;
                }
              }
